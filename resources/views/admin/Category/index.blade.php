...
{{--                                                    <td>{{ $item->category_name_bn }}</td>--}}
                                                    ...
                                                    <td>
                                                        <div class="input-group">
                                                            <a href="{{ route('categories.edit', $item) }}" class="btn btn-info" title="Edit Data" style="height: 34px; "><i class="fa fa-pencil"></i></a>
                                                            <form action="{{ route('categories.destroy', $item) }}" method="post">
                                                                @method('DELETE')
                                                                @csrf
                                                                <a href="" class="btn btn-danger" title="Delete Data" id="delete" onclick="event.preventDefault();
                                                                this.closest('form').submit();" style="margin-left:10px; "><i class="fa fa-trash"></i></a>
                                                            </form>
                                                        </div>
                                                    </td>
                                                </tr>
